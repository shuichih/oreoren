...
static obj_face* obj_parse_face(obj_growable_scene_data *scene)
{
	...
	int vertex_count = obj_parse_vertex_index(face->vertex_index, face->texture_index, face->normal_index);
...
static obj_vector* obj_parse_vector()
{
    ...
    char* tmp1 = strtok(NULL, WHITESPACE);
    char* tmp2 = strtok(NULL, WHITESPACE);
    char* tmp3 = strtok(NULL, WHITESPACE);
	v->e[0] = atof( tmp1 );
	v->e[1] = atof( tmp2 );
	v->e[2] = atof( tmp3 );
	return v;
...
static obj_vector* obj_parse_vector2()
{
	obj_vector *v = (obj_vector*)malloc(sizeof(obj_vector));
